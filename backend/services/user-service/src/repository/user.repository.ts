...
export class UserRepository {
  async getAdminOverviewSummary(): Promise<{
    totalUsers: number;
    ...
    const registrationCounts = Array.from({ length: 7 }, (_, index) => {
    ...
    const [
      ...
      prisma.user.count(),
      prisma.user.count({ where: { status: Status.APPROVED } }),
      prisma.user.count({ where: { status: Status.PENDING } }),
      prisma.user.count({ where: { status: Status.REJECTED } }),
