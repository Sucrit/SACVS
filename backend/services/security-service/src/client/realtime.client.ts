...
export interface RealtimeEvent {
  domain: 'users' | 'credentials' | 'credentialRequests' | 'notifications' | 'audit' | 'system' | 'security';
  action: string;
  entityId?: string;
  scope?: {
    broadcast?: boolean;
...
export class RealtimeClient {
  ...
  async publishMany(events: RealtimeEvent[]): Promise<void> {
    ...
    try {
      ...
      console.error('Failed to publish realtime event(s) from security-service:', error);
    }
  }
}
