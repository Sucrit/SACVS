...
export default function RequestReceiptVerifyPage() {
  ...
  return (
    <main className="credence-font min-h-screen bg-white px-4 py-8 sm:py-10">
      <div className="mx-auto w-full max-w-3xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_20px_45px_rgba(15,23,42,0.10)]">
        <div className="border-b border-slate-200 bg-slate-900 px-6 py-5 text-white sm:px-8">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-300">Public one-time validation for approved physical pickup requests.</p>
            </div>
            <button
              type="button"
              onClick={() => navigate('/')}
        ...
        <div className="space-y-5 px-6 py-6 sm:px-8 sm:py-7">
          {isLoading && (
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
              <p className="text-sm font-medium text-slate-600">Verifying approval receipt token...</p>
            </div>
          )}
